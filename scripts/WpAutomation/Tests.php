...
class Tests
{
    ...
    public static function install(Event $event, $pluginName = '')
    {
        ...
        if (!empty($pluginName)) {
            ...
            if (file_exists($pluginDir)) {
                ...
                $commantToRun = sprintf($tplCommand, $pluginDir);

                $command = new Command($commantToRun);
                if ($command->execute()) {
            ...
            foreach ($pluginsDir as $dir) {
                ...
                if ($dir->isDir() && file_exists($dir->getRealPath() . '/bin/install-wp-tests.sh')) {
                    ...
                    $commantToRun = sprintf($tplCommand, $dir->getRealPath());
    ...
    public static function run(Event $event)
    {
        ...
        if (count($args) > 0) {
            ...
            $commantToRun = "phpunit --debug --colors=always --configuration $pluginPath/phpunit.xml.dist";
            $command = new Command($commantToRun);
            if ($command->execute()) {
